...
# ── Keras compatibility patches (models trained on older Keras) ────────────
# Patch 1: batch_shape → input_shape
from keras.src.engine import input_layer as _il
_orig_il = _il.InputLayer.from_config.__func__

@classmethod
def _patch_il(cls, config):
    if 'batch_shape' in config:
        config = dict(config)
        config['input_shape'] = config.pop('batch_shape')[1:]
    return _orig_il(cls, config)

_il.InputLayer.from_config = _patch_il

# Patch 2: dtype policy received as str instead of Policy object
from keras.src.mixed_precision import policy as _mp

def _patch_get_policy(identifier):
    if isinstance(identifier, str):
        try:
            return _mp.Policy(identifier)
        except Exception:
            return _mp.Policy("float32")
    if isinstance(identifier, _mp.Policy):
        return identifier
    # Fallback: coerce to string and wrap, or default to float32
    try:
        return _mp.Policy(str(identifier))
    except Exception:
        return _mp.Policy("float32")

_mp.get_policy = _patch_get_policy
# ──────────────────────────────────────────────────────────────────────────
...
def _get_model(ticker: str):
    if ticker not in _model_cache:
        _model_cache[ticker] = load_model(
            f"{MODELS_DIR}/{ticker}.keras",
...
def prefetch_all_prices(coins: list, usd_to_inr: float) -> dict:
    ...
    for ticker in coins:
        try:
            ...
            print(f"  {ticker:<6} {float(price_data[ticker][-1]):>12,.2f} INR")
...
def predict_coin_from_prices(ticker: str, inr_prices: np.ndarray, usd_to_inr: float) -> dict:
    ...
    if data_max_ <= data_min_:
        raise ValueError(f"Corrupt scale file for {ticker}")
    ...
    if ratio > 10 or ratio < 0.1:
        scaler_warning = (
            ...
            f"({current_price:,.2f} INR). Consider retraining."
        )
...
def predict_all(selected_coins: list = None) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        future_map = {executor.submit(safe_predict, c): c for c in coins}
...
if __name__ == "__main__":
    ...
    print(f"\n{'COIN':<8} {'CURRENT':>15}  {'PREDICTED':>15}  {'UP%':>6}  {'DOWN%':>6}")
    print("=" * 65)
